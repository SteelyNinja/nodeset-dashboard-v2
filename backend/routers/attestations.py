...
@router.get("/nodeset-validator-accuracy")
async def get_nodeset_validator_accuracy(
...
@router.get("/nodeset-performance-trends")
async def get_nodeset_performance_trends(
    start_epoch: Optional[int] = Query(None, description="Start epoch (inclusive)"),
    end_epoch: Optional[int] = Query(None, description="End epoch (inclusive)")
) -> Dict[str, Any]:
...
@router.get("/nodeset-validator-details")
async def get_nodeset_validator_details(
...
async def get_operator_performance(
    ...
    """Get detailed operator performance metrics (legacy endpoint)"""
    ...
    try:
        results = clickhouse_service.get_operator_performance(operator)
